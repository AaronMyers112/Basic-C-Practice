...
typedef date_t *date_ptr;

bool date_read ( date_ptr dp )
{
	
	scanf("%d-%d-%d", dp->year, dp->month, dp->day);
	return true;
...
int date_compare ( date_ptr date1, date_ptr date2 )
{
    if (date1->year < date2->year)
    {
    	return -1;
    }
    else if (date1-> > date2->year)
    {
    	return 1;
    }
    else
...
bool is_leap_year ( date_ptr date)
{
	if (date.year % 4 == 0)
	{
		if (date.year % 100 == 0)
    ...
    if (date.year >= 1)
    {
    	if (1 <= date.month >= 12)
    	{
    		if(date.month == 1 || date.month == 3 || date.month == 5 || date.month == 7
    			|| date.month == 8 || date.month == 10 || date.month == 12 && date.date <= 31)
    		{
    			return true;
    		}
    		else if (date.month == 9 || date.month == 6 || date.month == 11 && date.date <= 30)
    		{
    			return true;
    		}
    		else if (is_leap_year == true && date.month == 2 && date.day <= 29)
    		{
    			return true;
    		}
    		else if( is_leap_year == false && date.month == 2 && date.day == 28)
    		{
    			return true;
    		}
    		else
